...
import com.gaofeicm.qqbot.controller.JpkWebSocketController;
 ...
 * @author Gaofeicm
 */
public class JpkMessageUtils {

    private final static Logger log = LoggerFactory.getLogger(JpkMessageUtils.class);
    ...
    private static void reConnection(String msg, String userId, String message){
        try {
            if(!JpkMessageUtils.webSocketClient.isOpen()){
                JpkWebSocketController.webSocketClient();
            }
            Thread.sleep(1 * 1000);
